...
#define PS2_READ_OUTPUT_PORT \
    0xD0 /* Reading Controller Output port, resp: the byte */
#define PS2_WRITE_OUTPUT_PORT 0xD1 /* Write next byte to the Output port */
#define PS2_WRITE_PORT_1_OUTPUT_BUFFER \
    0xD2 /* Appears like Port 1 sent next byte to us */
#define PS2_WRITE_PORT_2_OUTPUT_BUFFER \
    0xD3 /* Appears like Port 2 sent next byte to us */
#define PS2_WRITE_PORT_2_INPUT_BUFFER \
    0xD4 /* Tell PS/2 to send byte to Port 2 */
...
void ps2_toggle_port(int, bool);
bool ps2_test_port(int);
bool ps2_test_controller();
void ps2_send_command(uint8_t);
uint8_t ps2_get_data();
bool ps2_send_port1_data_ack(uint8_t);
bool ps2_send_port2_data_ack(uint8_t);
bool ps2_send_port1_data(uint8_t);
bool ps2_send_port2_data(uint8_t);
bool ps2_toggle_translation(bool);
bool ps2_reset_port(int);
const bool* get_ps2_device_status_list();
